...
function getTargetPlugins(pluginStore, pluginDeps) {
	const pluginsPathPattern = _getPluginsPattern(pluginStore, pluginDeps, '**/' + pluginInfoFile)
	logger.trace('get plugins pattern: ', pluginsPathPattern)

	const plugins = packageLoader.getPackages(pluginsPathPattern, _constructPluginConfig)

	if (pluginDeps && _.isEmpty(plugins)) {
 ...
 * @returns {string}
 */
function _getPluginsPattern(prefix, plugins, postfix) {
	if (plugins.length === 1) {
...
function getAllPlugins(pluginStore) {
	...
	return packageLoader.getPackages(pluginsPathPattern, _constructPluginConfig)
}
