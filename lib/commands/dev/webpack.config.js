 ...
 * @returns Object
 */
function constructDevDefaultConfig(config, context) {
	...
	return {
		...
		output: {
			...
			devtoolModuleFilenameTemplate: function (info) {
				return 'file://' + path.format(path.parse(info.absoluteResourcePath))
			}
		},
		debug: true,
		devtool: 'source-map',
		module: {
			loaders: [
				...
				// html
				{
					test: /\.html$/,
					exclude: /index\.html$/,
					loaders: ['ngtemplate?relativeTo=' + context, 'html?attrs=link:href img:src source:src']
				},
				// image file
				{
