...
const logger = require('./logger').getLogger('BowerInstaller')
...
function checkAndInstall(targetDependencies, option) {
	return diffBowerInstalledPackages(targetDependencies)
	...
	return new Promise(function (resolve, reject) {
		...
		bower.commands.list(null, {
			...
			.on('end', function (results) {
				...
				logger.debug('packages installed before are: ', results)
